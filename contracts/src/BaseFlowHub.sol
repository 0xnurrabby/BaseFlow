...
contract BaseFlowHub {
    ...
    event EmergencyETHRecovered(address indexed to, uint256 amount);
    ...
    function emergencyRecoverETH(address to) external onlyOwner nonReentrant {
        uint256 bal = address(this).balance;
        SafeTransferLib.safeTransferETH(to, bal);
        emit EmergencyETHRecovered(to, bal);
    }

    function emergencyRecoverToken(address token, address to) external onlyOwner nonReentrant {
        uint256 bal = IERC20(token).balanceOf(address(this));
        SafeTransferLib.safeTransfer(token, to, bal);
        emit EmergencyTokenRecovered(token, to, bal);
    }
}
